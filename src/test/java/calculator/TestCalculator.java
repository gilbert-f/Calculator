...
public class TestCalculator extends BaseTest {
    ...
    public void testToDouble() {
        ...
        assertEquals("17", calc.evaluate("toDouble(3 + 2 * 7)"));
        assertEquals("0", calc.evaluate("toDouble(2 - 4 / 2)"));
        assertEquals("1", calc.evaluate("x := 1"));
        assertEquals("2", calc.evaluate("toDouble(1 + x)"));
        assertEquals("2", calc.evaluate("y := 2"));
        assertEquals("3", calc.evaluate("toDouble(x + y)"));
        assertEquals("-1", calc.evaluate("toDouble(x + - y)"));
        assertEquals("4", calc.evaluate("2 ^ 2"));
        assertEquals("sin(4)", calc.evaluate("sin(4)"));
        assertEquals("-0.7568024953079282", calc.evaluate("toDouble(sin(4))"));
    }    
