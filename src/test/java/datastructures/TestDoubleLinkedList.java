...
public class TestDoubleLinkedList extends BaseTest {
    ...
    // delete tests
    @Test(timeout=SECOND)
    public void testDeleteBasic() { // happy case: trying to delete basically
        IList<String> list = this.makeBasicList();
        list.add("d");
        list.add("e");
        list.add("f");
        ...
        this.assertListMatches(new String[] {"b", "c", "d", "e", "f", "g", "h", "i"}, list);
        assertEquals(8, list.size());

        assertEquals("c", list.delete(1));
        this.assertListMatches(new String[] {"b", "d", "e", "f", "g", "h", "i"}, list);
        assertEquals(7, list.size());
        
        assertEquals("e", list.delete(2));
        this.assertListMatches(new String[] {"b", "d", "f", "g", "h", "i"}, list);
        assertEquals(6, list.size());
        
        assertEquals("i", list.delete(5));
        this.assertListMatches(new String[] {"b", "d", "f", "g", "h"}, list);
        assertEquals(5, list.size());
        
        assertEquals("g", list.delete(3));
        this.assertListMatches(new String[] {"b", "d", "f", "h"}, list);
        assertEquals(4, list.size());
        
        assertEquals("h", list.delete(3));
        this.assertListMatches(new String[] {"b", "d", "f"}, list);
        assertEquals(3, list.size());          
    }
    
    @Test(timeout=SECOND)
    public void testDeleteOnSingleElement() { // edge case: trying to delete from list with one element
        IList<String> list = this.makeBasicList();
        list.remove();
        list.remove();
    ...
    @Test
    public void testDeleteOutOfBounds() { // bad input: trying to pass an index that is out of bounds
        IList<String> list = this.makeBasicList();
    ...
    @Test(timeout=SECOND)
    public void testDeleteFromFront() { // edge case: trying to delete from front of list
        IList<Integer> list = this.makeInstance();
        int cap = 1000;
    ...
    @Test(timeout=SECOND)
    public void testDeleteFromEnd() { // edge case: trying to delete from end of list
        IList<Integer> list = this.makeInstance();
        int cap = 1000;
        ...
        for (int i = cap - 1; i >= 0; i--) {
        		assertEquals(i, list.delete(i));
        }

        assertEquals(0, list.size()); // check if the size is updated
    }    
}
